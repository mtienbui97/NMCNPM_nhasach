...
namespace GUI.SACH
{
    public partial class FormImportSach : Form
    {
        public FormImportSach()
        {
            InitializeComponent();
        }
        ...
        private void import_button_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                if (!row.IsNewRow)
                {
                    SachDTO temp = new SachDTO
                    {
                        ...
                        TheLoai = row.Cells["theloai"].Value == null ? "" : row.Cells["theloai"].Value.ToString(),
                        TacGia = row.Cells["tacgia"].Value == null ? "" : row.Cells["tacgia"].Value.ToString(),
                        DonGia = float.Parse(row.Cells["dongia"].Value == null ? "" : row.Cells["dongia"].Value.ToString()),
                    };
            ...
            if(SachBUS.InsertSach(listSach))
            {
